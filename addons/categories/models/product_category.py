...
class ProductCategory(BaseModel, KidsClothingMixin, PriceMixin):
    ...
    complete_name = CharField(string='Complete Name', size=500, readonly=True)
    parent_id = Many2OneField('product.category', string='Parent Category')
    child_id = One2ManyField('product.category', string='Child Categories')
    parent_path = CharField(string='Parent Path', size=500)
    ...
    # Category Properties
    is_main_category = BooleanField(string='Main Category', default=False)
    is_leaf_category = BooleanField(string='Leaf Category', readonly=True)
    category_code = CharField(string='Category Code', size=10)
    description = TextField(string='Description')
    image = ImageField(string='Category Image')
    image_medium = ImageField(string='Medium Image', readonly=True)
    image_small = ImageField(string='Small Image', readonly=True)
    ...
    total_sales = FloatField(string='Total Sales', digits=(12, 2), readonly=True)
    avg_rating = FloatField(string='Average Rating', digits=(3, 2), readonly=True)
    ...
    # SEO Fields
    meta_title = CharField(string='Meta Title', size=255)
    meta_description = TextField(string='Meta Description')
    meta_keywords = CharField(string='Meta Keywords', size=500)
    
    # Display Properties
    display_type = SelectionField(string='Display Type', selection=[
        ('normal', 'Normal'),
        ('page', 'Page'),
        ('group', 'Group'),
    ], default='normal')
