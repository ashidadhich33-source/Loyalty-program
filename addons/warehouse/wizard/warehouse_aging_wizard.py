# -*- coding: utf-8 -*-
"""
Kids Clothing ERP - Warehouse Aging Wizard
=========================================

Warehouse aging analysis wizard for kids clothing retail.
"""
...
class WarehouseAgingWizard(BaseModel):
    """Warehouse aging analysis wizard"""
    
    _name = 'warehouse.aging.wizard'
    _description = 'Warehouse Aging Wizard'
    _table = 'warehouse_aging_wizard'
    ...
    warehouse_id = Many2OneField(
        ...
        string='Warehouse',
        required=True,
        help='Warehouse for aging analysis'
    )
    ...
    age_group_filter = SelectionField(
        ...
        default='none',
        help='Age group filter for aging analysis'
    )
    ...
    seasonal_filter = SelectionField(
        ...
        default='none',
        help='Seasonal filter for aging analysis'
    )
    ...
    def action_create_aging_report(self):
        """Create aging report from wizard data"""
        for wizard in self:
            # Validate wizard data
            wizard._validate_wizard()
            
            # Create aging report
            aging_report = wizard._create_aging_report()
        
        return {
            'type': 'ir.actions.act_window',
            'name': 'Aging Report Created',
            'res_model': 'stock.aging',
            'res_id': aging_report.id,
            'view_mode': 'form',
            'target': 'new'
        }
    ...
    def _validate_wizard(self):
        ...
        if not self.name:
            errors.append("Aging report name is required")
        
        if not self.warehouse_id:
            errors.append("Warehouse is required")
        
        if not self.aging_date:
            errors.append("Aging date is required")
        
        if errors:
            raise ValueError('\n'.join(errors))
    
    def _create_aging_report(self):
        """Create aging report from wizard data"""
        # Create aging report
        aging_vals = {
            'name': self.name,
        ...
        aging_report = self.env['stock.aging'].create(aging_vals)
        
        # Generate aging data
        aging_report._generate_aging_data()
